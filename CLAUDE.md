...
- **Player-name matching:** deterministic/rules-based (exact match → suffix-normalized match → manual override table), not LLM-based. Keeps things fast and token-light — don't reach for a model call to reconcile player names. This still holds with an LLM in the pipeline elsewhere (see player news, below): Claude is only ever used to interpret external, unstructured content into a clean schema or to write a summary — it never resolves a name to this project's own canonical player_id. That boundary is deliberate, not an oversight.
- **NFL team codes:** every write of an NFL team abbreviation goes through `ffassistant.nfl_teams.canonical_team_code()` — sources disagree (ESPN `WSH`, Yahoo title-case, Sleeper/rankings `WAS`/`LA`) and the per-team reference tables (`nfl_team_byes`, `nfl_team_playoff_sos`, `nfl_team_schedule`, `nfl_team_implied_totals`) key off one spelling. Canonical = whatever those CSVs use (`WSH`, `LAR`, `LV`, `JAX`). Applied in all four ingest modules and the `scripts/import_*.py` reference-data importers; `players.nfl_team` is refreshed on every platform resync (not rankings — lower trust). `scripts/normalize_player_teams.py` backfills legacy rows.
...
Five peer sections, not nested inside each other: **Draft | In-season | Exposure | League settings | W-L**. Draft and In-season swap places in the nav depending on whether the season is currently active (per `ffassistant.season.smart_current_week()`) — Draft leads preseason, In-season leads once weeks 1-17 are underway — since whichever one is actually actionable right now should lead; Exposure stays the pivot between them since it's relevant year-round either way, and League settings/W-L (low-frequency admin/reference) stay last regardless.
...
"The season's current week" (used throughout in-season: this info card's opponent line, ROS's matchup fallback, the nav-ordering swap, the scheduled/on-demand refresh's default week) is figured out automatically via `ffassistant.season.smart_current_week()` — a live, no-setup lookup against Sleeper's public NFL-state endpoint (unauthenticated, independent of which platforms Keith's own leagues are actually on), treating preseason as week 1 since there's nothing meaningful to show before the season starts. The once-a-year manual week1_start_date setting in League Settings' Season panel (`ffassistant.season.current_week()`, pure date math) only kicks in as a fallback if that live lookup is ever unreachable.

Both views also open with a "This Week" info card — Keith's record, this week's opponent and their record, his current waiver priority, and a short recent-news digest on his rostered players — rendered once per view rather than per position, since these are team-level facts, not per-player ones. Record/opponent/waiver priority come from the platform sync (same three connectors, all already exposing wins/losses/ties and weekly opponent pairings); news comes from NBC Sports' "Rotoworld" player-news archive (public, not the rankings provider and not subject to its confidentiality rule) and is refreshed independently via its own sync. Each line is simply omitted, not shown blank, until its data has actually been synced. ROS has no week of its own, so its opponent line falls back to the season's current week.

**Player news is this project's first LLM usage** (`ffassistant/claude_news.py`, real Anthropic API calls — needs `ANTHROPIC_API_KEY` in the local `.env`, a separate cost/credential from anything Claude Code itself uses). The news page is scraped and generically size-reduced (`ffassistant/connectors/nbc_news.py` — strips scripts/styles/images/nav/header/footer, a site-agnostic cleanup, not a hardcoded field-level scrape), then Claude both (a) extracts structured items from that content and (b) synthesizes each rostered player's last few days of items into a short digest — both via `client.messages.parse()` with a Pydantic schema for guaranteed-shape output, rather than hoping for well-formed prose JSON. Deliberately routing extraction through Claude instead of hardcoded CSS selectors makes it resilient to NBC changing their markup later. Matching an extracted name back to a canonical `player_id`, though, stays entirely in the existing deterministic `name_matching` pipeline — see the Player-name matching bullet above.
...
## Verification checklist — rendered HTML/CSS work (recap_html.py, draft board JS, any generated UI)

Two real bugs shipped in `recap_html.py` before being caught by Keith rather than by testing (the Matchup Stories tab-highlight bug, and a styling-pattern request only being applied to the one element named instead of every analogous one). Both were checkable in advance without Keith's screenshot. Treat these as required steps, not nice-to-haves, whenever changing rendered HTML/CSS in this repo:

- **Render the actual output and grep-verify it, don't just read the template code.** Python string templates read as correct far more often than the HTML they produce actually is — the only way to know a change worked is to run the real render step (`scripts.render_weekly_recap` or equivalent) and grep the resulting file for the specific thing that changed (the new CSS rule, the new markup, the absence of the old one). This project's own tests only assert markup/text content, never CSS — a passing test suite does not mean the styling is right.
- **Before trusting any `:checked`/sibling-combinator CSS (radio-button tabs, toggle patterns), trace the actual DOM order the render function emits — don't assume adjacency from the code's argument order.** The tab bug happened because `_render_matchups` builds three flat runs (all `<input>`s, then all `<label>`s, then all panels) rather than interleaving each tab's input/label/panel together, so a class-based adjacent-sibling selector (`.input:checked + .label`) silently matched the wrong element. The fix pattern: generate one rule per element, scoped by that element's own id/`for` attribute, using the general-sibling combinator (`~`) — `#tab-N:checked ~ label[for="tab-N"]` — which works regardless of how the elements are grouped in the DOM or how they wrap at a given screen width. Prefer this id-scoped `~` pattern by default for any future toggle/tab UI here rather than a bare class-based `+`/`~` rule.
- **When asked to extend a color/spacing pattern ("use X for more things," "more space after subheaders"), grep the stylesheet for every element playing that visual role first, not just the one named in the request.** E.g. "subheaders" turned out to mean section `<h2>`s, the per-matchup `<h3>` headline, and `.side-subhead` together — fixing only the one Keith happened to point at would have left the other two inconsistent and prompted a follow-up round.

## Network access from Claude sandboxes (ESPN, GroupMe, and any future external API)

Both the cloud session and the linked device's shell (`device_bash`) go through their own separate network-egress proxy, each enforcing a domain allowlist — a call to a host that isn't on it fails at the proxy (403/connection-refused) before it ever reaches the real API, which looks identical to a genuine outage or credential problem if you don't know to check for it. If a new external API integration ever hits a mystery connection failure from either sandbox, check the Claude desktop app's **Admin settings → Capabilities** (a **restart of the app is required** for a change there to actually take effect — the first retry immediately after changing it still failed) before assuming the API/credentials are the problem. Both ESPN's and GroupMe's hosts have been added there and confirmed genuinely reachable (2026-09-15) — a direct call now returns a real API response, not a proxy block. See DECISIONS.md for the full trail on both.

**Even with the network open, don't run ESPN's actual data-writing syncs (`sync_box_scores`, `sync_weekly_matchups`, `sync_league`, etc.) through `device_bash` against the real database.** Doing exactly that during the Capabilities testing above triggered a second, separate problem: the connected folder's network mount doesn't reliably support the file-locking SQLite needs mid-write, and a moderately-sized sync left a stuck rollback journal that blocked all DB access (even reads) until Keith opened the file *natively* on his machine (`scripts/check_db_health.py`/`.bat`) and let SQLite auto-recover it — same class of issue as the git `.git/index.lock` note elsewhere in this file. ESPN syncs belong on a **native Windows Scheduled Task** on Keith's own machine (already registered, per `sync_week_results.py`/`refresh_in_season.py`'s docstrings) — that path never touches the Claude sandbox network *or* the device-bridge mount, so neither of these issues can happen there. `device_bash` remains fine for read-only queries and for driving the recap-building scripts (`prepare_weekly_recap`/`render_weekly_recap`), which only read already-synced data. GroupMe's `post_message` is a one-off, tiny write with no comparable transaction size, so running `--post-groupme` from a live session is fine.
