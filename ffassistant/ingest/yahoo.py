"""Sync a Yahoo league's settings, teams, current rosters, and injury status into the database.
...
def sync_league(
    ...
    """league_id is this project's internal leagues.league_id; yahoo_league_id is Yahoo's own ID
    (e.g. '461.l.656302'). `week` is optional — pass it during an in-season sync to also record
    each player's current injury status for that week.
    """
    _sync_settings(conn, league_id, yahoo_league_id)
    _sync_teams_and_rosters(conn, league_id, yahoo_league_id, season, week)
    conn.commit()


def _sync_settings(conn: sqlite3.Connection, league_id: int, yahoo_league_id: str) -> None:
    settings = yahoo_api.get_league_settings(yahoo_league_id)
...
def _sync_teams_and_rosters(
    conn: sqlite3.Connection, league_id: int, yahoo_league_id: str, season: int, week: int | None
) -> None:
    teams = yahoo_api.get_teams(yahoo_league_id)
        ...
        for player_info in team["players"]:
            ...
            if week is not None:
                conn.execute(
                    ...
                    (player_id, season, week, player_info["injury_status"]),
                )

    if week is not None:
        _sync_matchups(conn, league_id, yahoo_league_id, season, week, team_id_by_platform_id)


def _sync_matchups(conn, league_id, yahoo_league_id, season, week, team_id_by_platform_id) -> None:
    pairs = yahoo_api.get_matchups(yahoo_league_id, week)
